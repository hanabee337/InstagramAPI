"""
2017.03.16 : InstagramAPI DRF Refactoring 1
1. Post models의 to_dict 메서드 삭제
2. test 패키지를 생성, 패키지 내부에 tests_apis.py 생성
    Post Create, Photo Add를 각각 테스트 케이스로 구현
   실행할 동작을 테스트 코드로 구현
3. 테스크 코드가 하나씩 돌아갈 수 있도록 코드를 구현
    PostList:
        Post에 대한 List, Create
    PostDetail:
        Post에 대한 Retrieve, Update, Destroy

    3-1. serializers.py파일 생성, Post에 Photo에 대한 serializer 구현
